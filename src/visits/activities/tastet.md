---
layout: layouts/visit-activity.njk
name: visits
footerVariant: visits-newsletter
tags:
  - visitActivities
order: 4
title: TASTET
permalink: /visits/tastet/
cardImage: /assets/images/gallery/visites/visites_guiades_1.jpg
cardImageAlt: Tastet de productes de castanya
cardDescription: Experimenta en un tast tot el mon gourmet que te la castanya. A traves de 5 productes locals, tots amb castanya.
heroImage: /assets/images/gallery/visites/visites_guiades_1.jpg
heroImageAlt: Taula de tast de productes de castanya
galleryButtonLabel: VEURE GALERIA DE FOTOS
reservationPrompt: ENVIA UNA SOL.LICITUD DE RESERVA
reservationPrimaryLabel: RESERVA EL TEU TASTET
reservationPrimaryLink: /contact/
whatsappLink: https://wa.me/34608131199?text=Hola%2C%20m%27interessa%20el%20Tastet%20de%20productes%20de%20castanya.
emailAddress: info@castanyadeviladrau.cat
phoneNumber: +34 608 13 11 99
bookingHighlights:
  - PROPOSTA CURTA I MOLT VERSATIL
  - PERFECTA COM A COMPLEMENT D'ALTRES EXPERIENCIES
personsMin: 2
personsMax: 8
duration: 45 min
priceFrom: 20EUR
priceNote: segons nombre de persones
difficulty: Nivell facil
kidFriendly: Apte per nens
languages: Cat - Esp - Ang
overviewIntro: Una degustacio comentada per descobrir textures, aromes i aplicacions diferents de la castanya a traves de productes seleccionats del territori.
itinerary:
  - time: 12.00H
    title: Presentacio dels productes
    description: Introduccio als formats, procedencies i maridatges possibles.
  - time: 12.10H - 12.35H
    title: Tast guiat
    description: Degustacio de cinc productes amb explicacio de sabors, usos i particularitats.
  - time: 12.35H - 12.45H
    title: Tancament i recomanacions
    description: Espai per resoldre dubtes i descobrir idees per integrar la castanya en el dia a dia.
pricing:
  - group: 2
    price: 25EUR
  - group: 4
    price: 22EUR
  - group: 6
    price: 20EUR
  - group: 8
    price: 18EUR
includedServices: |
  - Tast guiat de cinc productes amb castanya.
  - Explicacio de processos, ingredients i aplicacions.
  - Recomanacions de maridatge i conservacio.
meetingPoint: |
  El tast es pot fer a la finca o en un espai acordat dins el projecte, segons disponibilitat.

  T'enviarem la informacio definitiva amb la reserva.
additionalInfo: |
  - Si hi ha intolerancies o necessitats alimentaries, avisa'ns amb antelacio.
  - El tastet es pot combinar amb una visita guiada o una proposta privada.
experienceHighlightsSection:
  eyebrow: ENRIQUEIX LA TEVA AVENTURA AL MONTSENY
  title: ENRIQUEIX LA TEVA AVENTURA AL MONTSENY
  intro: Detalls que fan que cada visita a la castanyeda sigui única.
  items:
    - title: LA CASTANYERA
      image: /assets/images/gallery/visites/visites_guiades_1.jpg
      imageAlt: Castanyera al Montseny
    - title: EXPERIENCIA ANTIESTRES
      image: /assets/images/gallery/visites/visites_guiades_2.jpg
      imageAlt: Detall de castanyer al bosc
    - title: APAT
      image: /assets/images/gallery/visites/visites_guiades_3.jpg
      imageAlt: Apat amb productes de castanya
    - title: ARBRE DE CASTANYER
      image: /assets/images/gallery/visites/visites_guiades_banner.jpg
      imageAlt: Arbre de castanyer
reviewsSection:
  eyebrow:
  title:
  ratingLabel: ★ 4,79, · 1506 EVALUACIONS DE LA VISITA
  ratingValue:
  sourceLabel:
  reviews:
    - quote: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.
      author: Maria Sheferd
      date: 19/04/2024
      avatar: /assets/images/logo-maria.png
      avatarAlt: Maria Sheferd
    - quote: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.
      author: Maria Sheferd
      date: 19/04/2024
      avatar: /assets/images/logo-maria.png
      avatarAlt: Maria Sheferd
    - quote: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.
      author: Maria Sheferd
      date: 19/04/2024
      avatar: /assets/images/logo-maria.png
      avatarAlt: Maria Sheferd
gallery:
  - image: /assets/images/gallery/visites/visites_guiades_3.jpg
    alt: Productes artesans amb castanya
  - image: /assets/images/gallery/visites/visites_guiades_1.jpg
    alt: Espai de tast a la finca
  - image: /assets/images/gallery/visites/visites_guiades_banner.jpg
    alt: Ambient de visita a mida
---

El tastet es una manera directa i saborosa d'entrar a l'univers de la castanya de Viladrau. En poca estona, condensem el mes essencial del projecte en una degustacio cuidada i propera.

La sessio esta pensada per a persones curioses, grups reduits i visites que busquen una primera aproximacio gastronomica al nostre treball.

### Hi trobaras

- Productes dolcos i salats elaborats amb castanya.
- Explicacions senzilles sobre origen, transformacio i possibles usos.
- Un format agil, ideal per complementar altres activitats o visites curtes.
