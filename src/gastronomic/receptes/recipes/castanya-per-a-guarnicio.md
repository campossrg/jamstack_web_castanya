---
layout: layouts/recipe.njk
name: gastronomic-recipes
footerVariant: visits-newsletter
tags:
  - recipes
title: CASTANYA PER A GUARNICIO
order: 4
permalink: /gastronomic/receptes/castanya-per-a-guarnicio/
listingSections:
  - foundations
image: /assets/images/gallery/espai-gastronomic/castanya_camp.jpg
imageAlt: Castanya al paisatge del Montseny
summary: Una guarnicio simple i efectiva per acompanyar carns, verdures rostides o plats de cullera amb un toc de bosc i temporada.
author: Joan Xalet
origin: del Xalet la Coromina
time: 15 minuts
difficulty: facil
ingredientsLabel: INGREDIENTS
ingredients:
  - Castanya fresca
  - Sal fina
  - Oli d'oliva verge extra
  - Farigola
downloadPdfLabel: DESCARREGAR RECEPTA
pdfEnabled: true
steps:
  - title: PAS 1
    text: Fes un tall petit a cada castanya i escalda-les breument per facilitar-ne el pelat. Un bon punt de cocció inicial t'estalvia feina mes endavant.
  - title: PAS 2
    text: Salta-les amb un raig d'oli d'oliva i una mica de farigola fins que quedin daurades per fora i tendres per dins.
  - title: PAS 3
    text: Afegeix la sal al final i serveix-les calentes al costat del plat principal. Son ideals per donar textura i un punt aromatic de tardor.
secretTitle: EL SECRET DEL JOAQUIM
secretQuote: Si no les cuines massa, mantenen millor la textura i fan de guarnicio sense convertir-se en pure.
---

Una opcio molt util per convertir la castanya en un complement natural de molts plats de temporada.
