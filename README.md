...
4. **Environment variables**
   ...
   ```env
    ...
    TINA_TOKEN=your_tina_readonly_token
   ```
...
2. **Connect to Netlify**
   ...
   - Add environment variables: `TINA_CLIENT_ID`, `TINA_TOKEN`

3. **Connect TinaCloud**
   - Create a project in TinaCloud and connect the GitHub repository
   - Set the production branch to `main`
   - Enable direct commits to `main` for editorial changes
   - Copy the TinaCloud `Client ID` and `Read Only Token` into Netlify environment variables
   - In TinaCloud, add your Netlify production URL to the allowed preview/admin origins if prompted
...
5. **Verify deployed admin**
   - Open `https://your-site.netlify.app/admin/`
   - Sign in through TinaCloud
   - Edit a document and save it
   - Confirm Tina creates a commit directly on `main` in GitHub and Netlify starts a new deploy
...
### TinaCMS Production Notes
- Production admin assets are generated by `pnpm tina:build` and then served from `/admin/`.
- This Eleventy site uses Tina in sidebar/document editing mode rather than full React visual editing.
- Repo-side wiring is only part of the setup: TinaCloud project configuration and GitHub authorization must be completed in the Tina dashboard.
